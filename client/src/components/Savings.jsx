...
import { formatAmount, calculateFutureValue } from "../components/utils/formaDate";

const  Savings=({ currentMonthHighLevel }) =>{

function getNonEssential() {
    if (!currentMonthHighLevel || !Array.isArray(currentMonthHighLevel)) {
     
      return 0; 
    }
  
    const nonEssentialEntry = currentMonthHighLevel.find(
      (entry) => entry.highLevelCategory === "Non-Essential"
    );
  
    if (!nonEssentialEntry) {
      // Handle the case where the "Non-Essential" category is not found.
      return 0; // or another default value
    }
  
    const nonEssentialSpending = Math.round(nonEssentialEntry.amount);
